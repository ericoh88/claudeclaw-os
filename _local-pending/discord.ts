...
async function handleDiscordMessage(msg: Message, content: string, voiceReply = false): Promise<void> {
  ...
  try {
    ...
    const memCtx = await buildMemoryContext(chatIdStr, content);
    ...
    // Subtype guard: a non-success result (error_max_turns, error_during_execution,
    // etc.) means the session is in a state that will fail again on resume. Clear
    // the mapping so the next message starts fresh.
    if (result.subtype && result.subtype !== 'success') {
      logger.warn(
        { chatIdStr, sessionId, subtype: result.subtype },
      ...
      const reason = reasonMap[result.subtype] ?? `failed (${result.subtype})`;
...
export async function initDiscord(): Promise<void> {
  ...
  discordClient.on('messageCreate', async (msg: Message) => {
    ...
    const nonVoiceAttachments = msg.attachments.filter(a =>
